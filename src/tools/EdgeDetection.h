...
public:
  ...
  static void laplacianFilter(std::unique_ptr<Image> &image,
                              int kernelSize = 3);

  // Operator Laplace'a z konwersją na skalę szarości dla lepszej wizualizacji
  static void laplacianFilterGrayscale(std::unique_ptr<Image> &image,
                                       int kernelSize = 3);

  // Negatywny operator Laplace'a (wykrywa ciemne linie na jasnym tle)
  static void laplacianFilterNegative(std::unique_ptr<Image> &image,
                                      int kernelSize = 3);
  // Laplacian of Gaussian (LoG) - metoda z wygładzaniem Gaussowskim
  static void laplacianOfGaussian(std::unique_ptr<Image> &image,
                                  double sigma = 1.0, int windowSize = 3,
                                  double threshold = 0.1);

  // Uproszczona wersja LoG bez progowania (dla testów)
  static void laplacianOfGaussianSimple(std::unique_ptr<Image> &image,
                                        double sigma = 1.0);
  ...
  static void cannyEdgeDetection(std::unique_ptr<Image> &image,
                                 double upperThresh = 100.0,
                                 double lowerThresh = 50.0) {
    Canny::applyCanny(image, upperThresh, lowerThresh);
  }

  // Filtr wykrywania krawędzi z macierzą niestandardową
  static void customEdgeFilter(std::unique_ptr<Image> &image,
...
private:
  ...
  generateLaplacianNegativeKernel(int size);

  // Generowanie jądra Laplacian of Gaussian
  static std::vector<std::vector<double>> generateLoGKernel(double sigma,
                                                            int size);
  // Aplikowanie algorytmu LoG z progowaniem zgodnie z instrukcją
  static void applyLoGWithThresholding(std::unique_ptr<Image> &image,
                                       double sigma, int windowSize,
                                       double threshold);

  // Generowanie jąder gradientowych
  static std::pair<std::vector<std::vector<double>>,
                   std::vector<std::vector<double>>>
  generateRobertsKernels();
  static std::pair<std::vector<std::vector<double>>,
                   std::vector<std::vector<double>>>
  generatePrewittKernels();
  static std::pair<std::vector<std::vector<double>>,
                   std::vector<std::vector<double>>>
  generateSobelKernels();

  // Aplikowanie konwolucji gradientowej
  static void
  applyGradientConvolution(std::unique_ptr<Image> &image,
  ...
  static void applyConvolution(std::unique_ptr<Image> &image,
                               const std::vector<std::vector<double>> &kernel);

  // Aplikowanie konwolucji Laplaciana z konwersją na skalę szarości
  static void applyLaplacianGrayscaleConvolution(
